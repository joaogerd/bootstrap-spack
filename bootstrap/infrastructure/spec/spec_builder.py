...
def _build_mpi_spec(pkg: DetectedPackage) -> str:
    name = _mpi_name_for_spec(pkg)
    version = pkg.metadata.get("version")
    return name + (f"@{version}" if version else "")


def _build_hdf5_spec(pkg: DetectedPackage, detected: Dict[str, DetectedPackage]) -> str:
    spec = _base_spec(pkg)
...
def _build_netcdf_c_spec(pkg: DetectedPackage, linkage: PackageLinkage, detected: Dict[str, DetectedPackage]) -> str:
    spec = _base_spec(pkg)
...
def _build_netcdf_fortran_spec(pkg: DetectedPackage, linkage: PackageLinkage, detected: Dict[str, DetectedPackage]) -> str:
    spec = _base_spec(pkg)
...
def _build_spec(
    ...
    if pkg.name in ("openmpi", "mpi", "mpich"):
        spec = _build_mpi_spec(pkg)
    elif pkg.name == "hdf5":
        spec = _build_hdf5_spec(pkg, detected)
    elif pkg.name == "netcdf-c":
        spec = _build_netcdf_c_spec(pkg, linkage, detected)
    elif pkg.name == "netcdf-fortran":
        spec = _build_netcdf_fortran_spec(pkg, linkage, detected)
    else:
        spec = _base_spec(pkg)
